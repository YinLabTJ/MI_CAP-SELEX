...
#4-mer A comes from TF1 motif and 4-mer B comes from TF2 motif, their distribution to MI will be removed if the Hudding distance between them is no more than 1
open(IN,"$shdir/../data/$skmer\-mer.xls") or die "$skmer-mer.xls file lost $!\n";
while(<IN>){
	chomp;
	@mer_t=split;
	if($mer_t[0] eq $tf[0]){
...
`mv $s1dir/MI.tmp2.xls $s1dir/MI.$tf_pair.$pair_8.$tf.sort.xls`;
...
close IN;
open OUT,">$s1dir/MI.$tf_pair.$pair_8.$tf.xls";
