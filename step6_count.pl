...
for($i=1;$i<=$lkmer-$skmer+1;$i++){
	...
	unless(($te1{$seq1}==1 && $te2{$seq1}==1) || ($te1{$seq3}==1 && $te2{$seq3}==1) || $degenerate2{$seq1}==1 || $degenerate1{$seq3}==1){
		...
		print MP "$comb1\t$comb2\t$comb3\t$comb4\t$comb5\t$comb6\t$comb7\t$comb8\n";
	}
}
...
while(<IN>){
	...
	if($hash{$pair}==1){
		...
		print TMP "$pair\t$tmp\t$s[1]\t$single{$key1}\t$single{$key2}\t$total\n" if($t[2]==1 && $t[3]==6);
	}
...
`mv $s1dir/MI.tmp2.xls $s1dir/MI.$te_pair.$pair_8.$tf.sort.xls`;
...
close TMP;
open OUT,">$s1dir/MI.$te_pair.$pair_8.$tf.xls";
