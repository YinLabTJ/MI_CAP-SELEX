...
my $dir=cwd;
$seq=shift; $pair_8=shift; $te_pair=shift; $pair_8s=shift; $lkmer=shift; $skmer=shift; $s1dir=shift; $tf=shift;
$wc=`wc -l $seq`;
@t=split /\s/,$wc;
$total=$t[0];
@t=split /_/,$pair_8;
...
$palindrome=0;
open MP,">mer_pair.xls";
@s=split /_/,$pair_8s;
@s1=split /,/,$s[0];
@s2=split /,/,$s[1];

@te=split /_/,$te_pair;
open(IN, "$dir/../$skmer-mer.xls") or die "k-mer.xls file lost $!";
while(<IN>){
        chomp;
        @mer_t=split;
        if($mer_t[0] eq $te[0]){
...
for($j=0;$j<@s1;$j++){
	for($k=0;$k<@s2;$k++){
		for($i=1;$i<=$lkmer-$skmer+2;$i++){
			...
			$seq1r=~tr/ATCG/TAGC/;
			$seq2r=~tr/ATCG/TAGC/;
			$te1{$seq1}=1; $te1{$seq1r}=1;
			$te2{$seq2}=1; $te2{$seq2r}=1;
		}
	}
}
...
open MER,">mer_pair.xls";
for($i=1;$i<=$lkmer-$skmer+1;$i++){
	$seq1=substr($t[0],$i-1,$skmer);
	$seq2=substr($t[1],$i,$skmer);
	$seq1r=reverse($seq1);
	$seq1r=~tr/ATCG/TAGC/;
	$seq2r=reverse($seq2);
	$seq2r=~tr/ATCG/TAGC/;
	unless(($te1{$seq1}==1 && $te2{$seq1}==1) || ($te1{$seq2}==1 && $te2{$seq2}==1) || $degenerate2{$seq1}==1 || $degenerate1{$seq2}==1){
		$comb1=$seq1."_".$seq2;
		$comb4=$seq2r."_".$seq1r;
		...
		$input[0]=$t[0]; $input[1]=$comb4; $huddinge1=Huddinge(@input);
		$input[0]=$t[1]; $input[1]=$comb4; $huddinge2=Huddinge(@input);
		if($huddinge1>1 && $huddinge2>1){ $hash{$comb4}=1; $hash1{$comb4}=1; }
		print MER "$comb1\t$comb4\n";
	}

	$seq1=substr($t[0],$i,$skmer);
	...
	unless(($te1{$seq1}==1 && $te2{$seq1}==1) || ($te1{$seq2}==1 && $te2{$seq2}==1) || $degenerate2{$seq1}==1 || $degenerate1{$seq2}==1){
		...
		$input[0]=$t[0]; $input[1]=$comb3; $huddinge1=Huddinge(@input);
		$input[0]=$t[1]; $input[1]=$comb3; $huddinge2=Huddinge(@input);
		if($huddinge1>1 && $huddinge2>1){ $hash{$comb3}=1; $hash2{$comb3}=1; }
		print MER "$comb2\t$comb3\n";
	}
	
	$revs=reverse($t[1]);
	$revs=~tr/ATCG/TAGC/;
	$seq1=substr($t[0],$i-1,$skmer);
	$seq3=substr($revs,$i,$skmer);
	$seq1r=reverse($seq1);
	$seq1r=~tr/ATCG/TAGC/;
	$seq3r=reverse($seq3);
	$seq3r=~tr/ATCG/TAGC/;
	unless(($te1{$seq1}==1 && $te2{$seq1}==1) || ($te1{$seq3}==1 && $te2{$seq3}==1) || $degenerate2{$seq1}==1 || $degenerate1{$seq3}==1){
		$comb5=$seq1."_".$seq3;
		$comb8=$seq3r."_".$seq1r;
		$input[0]=$t[0]; $input[1]=$comb5; $huddinge1=Huddinge(@input);
		$input[0]=$t[1]; $input[1]=$comb5; $huddinge2=Huddinge(@input);
		if($huddinge1>1 && $huddinge2>1){ $hash{$comb5}=1; $hash3{$comb5}=1; }
		$input[0]=$t[0]; $input[1]=$comb8; $huddinge1=Huddinge(@input);
		$input[0]=$t[1]; $input[1]=$comb8; $huddinge2=Huddinge(@input);
		if($huddinge1>1 && $huddinge2>1){ $hash{$comb8}=1; $hash3{$comb8}=1; }
		print MER "$comb5\t$comb8\n";
	}
...
open IN,"$s1dir/step1.$tf.single.out";
while(<IN>){
	chomp;
	@t=split;
	$single{$t[0]}=$t[1];
...
close IN;
open IN,"$s1dir/step1.$tf.pair.out";
open TMP,">$s1dir/tmp"; open TMP2,">$s1dir/tmp2";
$max=0;
while(<IN>){
	chomp;
	@s=split;
	...
	if($hash{$pair}==1){
		...
		print TMP "$pair\n" if($t[2]==32 && $t[3]==37);
		print TMP "1:$fenshu=$s[1]*$total/($single{$key1}*$single{$key2})\n" if($t[2]==32 && $t[3]==37);
		print TMP "2:$mi{$t[2]}{$t[3]}+=$s[1]*log($fenshu)/log(2)\n" if($t[2]==32 && $t[3]==37);
	}
	if($hash1{$pair}==1){$fenshu=$s[1]*$total/($single{$key1}*$single{$key2}); $mi1{$t[2]}{$t[3]}+=$s[1]/$total*log($fenshu)/log(2); print TMP2 "1\n"; }
	if($hash2{$pair}==1){$fenshu=$s[1]*$total/($single{$key1}*$single{$key2}); $mi2{$t[2]}{$t[3]}+=$s[1]/$total*log($fenshu)/log(2); print TMP2 "2\n"; }
	if($hash3{$pair}==1){$fenshu=$s[1]*$total/($single{$key1}*$single{$key2}); $mi3{$t[2]}{$t[3]}+=$s[1]/$total*log($fenshu)/log(2); print TMP2 "3\n"; }
	if($hash4{$pair}==1){$fenshu=$s[1]*$total/($single{$key1}*$single{$key2}); $mi4{$t[2]}{$t[3]}+=$s[1]/$total*log($fenshu)/log(2); print TMP2 "4\n"; }
}
close IN;
...
`cd $s1dir && perl $dir/../create_anova.pl && Rscript $dir/../oneway-anova.R > oneway-anova.out && Rscript $dir/../welchs-anova.R > welchs-anova.out && cd $s1dir/..`;
...
`cd $s1dir && perl $dir/../top_distance.pl > top_distance.xls && cd $s1dir/..`;
...
close TMP;
open OUT,">$s1dir/MI.$te_pair.$pair_8.$tf.further.xls";
